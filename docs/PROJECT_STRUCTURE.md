...
│   ├── App.tsx                       # QueryClientProvider, Router, Routes
...
| `user.ts`   | `User` (id, name, department, email, createdAt), `UserListParams`, `CreateUserRequest`, `UpdateUserRequest`.                                           |
...
- `users`: title, list, addUser, editUser, name, department, email, noData, notFound, search, actions, createSuccess, updateSuccess, deleteSuccess, deleteConfirm, required, invalidEmail, backToList
- `layout`: logout
...
- `MOCK_LOGIN`: `{ username: 'admin', password: 'admin123' }` — 이 계정으로만 로그인 성공.
...
| **Sidebar**        | `NavLink`로 대시보드, 사용자, 출입 로그, 부서, 설정(출입/네트워크/시스템) 링크. 활성 경로는 Tailwind로 강조.        |
...
| `/users`, `/users/new`, `/users/:id` | UsersPage           | 목록(검색·페이지네이션), 등록/수정 폼. TanStack Query, RHF+Zod. 저장 성공 시 목록으로 이동. 에러·로딩 피드백. |
...
- **🎭 목업 로그인**: MSW/Vite 목업 사용 시 `1` / `1` (정의: `src/mocks/handlers.ts`의 `MOCK_LOGIN`).
