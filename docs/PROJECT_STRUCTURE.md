...
- **UI**: 사용자 목록·폼·버튼 등 모두 MUI(Material UI) 사용.
...
hid-amico-web/
├── 📂 public/                           # 정적 자산 (그대로 출력)
│   ├── vite.svg                        # 파비콘/ favicon
│   └── mockServiceWorker.js            # MSW 서비스 워커 (npx msw init ./public --save 로 생성)
├── 📂 src/
│   ├── 📂 api/                          # API 레이어
...
├── 📂 vite/
│   └── mockApi.ts                   # Vite 개발 서버용 API 목업 미들웨어 (백엔드 없을 때 /api 처리)
├── index.html                       # SPA 진입점 HTML, root div, /src/main.tsx 로드
├── package.json                     # 의존성·스크립트·프로젝트 메타 (npm scripts, msw workerDirectory 등)
├── tsconfig.json                    # TypeScript 프로젝트 루트, tsconfig.app/node 참조, paths(@/*)
├── tsconfig.app.json                # 앱 소스(src)용 TS 설정 (빌드·린트, JSX, strict, include: src)
├── tsconfig.node.json               # Node 환경용 TS 설정 (vite.config.ts, vite/ include)
└── vite.config.ts                   # Vite 빌드 설정 (react, tailwind, api-mock, proxy, alias)
```
...
| 파일 / 폴더 | 용도 |
| ----------- | ----- |
| **index.html** | SPA 단일 HTML. `<div id="root">` + `type="module"` 로 `/src/main.tsx` 로드. 메타, 뷰포트, 파비콘(vite.svg). |
| **package.json** | 프로젝트 이름·버전, `dependencies`/`devDependencies`, `scripts`(dev/build/preview/lint). `msw.workerDirectory`로 MSW 워커 경로(public) 지정. |
| **tsconfig.json** | TypeScript 프로젝트 루트. `references`로 `tsconfig.app.json`, `tsconfig.node.json` 참조. `paths`: `@/*` → `./src/*`. |
| **tsconfig.app.json** | **앱 소스(`src`) 전용** 컴파일 옵션. target ES2022, DOM lib, JSX react-jsx, `include: ["src"]`, strict·noUnused·noEmit 등. Vite 빌드 시 사용. |
| **tsconfig.node.json** | **Node 환경** 전용 (Vite 설정·미들웨어). `include`: `vite.config.ts`, `vite/`. types: node. Vite가 설정 파일 타입 체크 시 사용. |
| **vite.config.ts** | Vite 설정. 플러그인: react, tailwindcss, api-mock(개발 시 `/api` 목업). `resolve.alias`: `@` → `src`. `VITE_USE_REAL_API=true`일 때만 `/api` 프록시. |
| **public/** | 정적 자산. `vite.svg`(파비콘), `mockServiceWorker.js`(MSW 워커, `npx msw init ./public --save`로 생성). |
| **vite/** | Vite 전용 스크립트. `mockApi.ts`: 개발 서버 미들웨어로 `/api` 요청 가로채서 목업 응답 반환. |
| **docs/** | 프로젝트 문서. `PROJECT_STRUCTURE.md`: 디렉터리·파일 구조 및 영역별 상세 설명. |
